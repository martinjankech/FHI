...
"awardet_for":"best performance by an Actress"}]
    ...
    test('It should response the GET method', () => {
        ...
        {   const app = new App();
            return request(app.getServer()).get("/movies").then(response => {
    ...
    test('responds with JSON array', () => {
         ...
         {   const app = new App();
            return request(app.getServer()).get('/movies')
            .then(response => { 
                expect(response.status).toBe(200);
        ...
        {   const app = new App(); 
            return request(app.getServer()).get('/movies/5fcd0a9f721d18579c879855')
            .then(res => {
      ...
      it('posts new book', () => {
        ...
        {   const app = new App();
           return  request(app.getServer()).post('/movies').send(testMovie)
            .then(res => {
                expect(res.status).toBe(200);
                expect(res.type).toEqual("application/json");
